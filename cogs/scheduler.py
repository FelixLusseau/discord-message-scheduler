...
DB_VERSION = 1
# options: 'dateutil', 'dateparser'
TIME_PARSE_METHOD: Literal["dateparser"] | Literal["dateutil"] = "dateparser"
...
class StrippedSavedScheduleEvent:
    ...
    # type: ignore[reportIncompatibleMethodOverride]
    def __lt__(self, other: StrippedSavedScheduleEvent) -> bool:
        """
        Use next_event_time as the comp.
        """
        return self.next_event_time < other.next_event_time
...
def _parse_repeat(raw_repeat: str | None) -> float | None:
    ...
    if not raw_repeat:
        ...
        try:
            ...
            # 12 seconds for debug mode, 60 min for production
            elif repeat < (0.2 if DEBUG_MODE else 60):
                if DEBUG_MODE:
                    raise InvalidRepeat(
                        "Repeat cannot be less than 12 seconds (debug mode is active).")
                else:
...
def get_schedule_modal(defaults: RawScheduleModalValues | None = None) -> Type[ScheduleModal]:
    ...
    timezone_default = cast(
        str, defaults and defaults.timezone or DEFAULT_TIMEZONE)
    ...
    class ScheduleModal(discord.ui.Modal, title="Schedule Creator"):
        ...
        def sanitize_response(self, interaction: discord.Interaction) -> SanitizedScheduleEvent:
            ...
            if self.time.value is None or self.message.value is None:
                raise ValueError(
                    "time and message cannot be None here since they are non-optional.")

            if not isinstance(interaction.user, discord.Member):
                raise ValueError(
                    "interaction.user must be a Member (cannot be ran from DM).")
            ...
            if TIME_PARSE_METHOD == "dateutil":
                ...
                if self.timezone.value:  # if user inputted a timezone
                    ...
                    # will use either tz from naive time or UTC
            ...
            if time <= now:
                logger.debug(
                    "Time is in the past. Time: %s, now: %s", time, now)
        ...
        def acceptable_formats(self) -> list[str]:
            ...
            if TIME_PARSE_METHOD == "dateutil":
                ...
                return [
                    "- Just date: `24/2/2023` (Day/Month/Year), `December 12`, `nov 26 2023`",
        ...
        async def on_submit(self, interaction: discord.Interaction) -> None:
            ...
            try:
                ...
                if mentions is not None:
                    ...
                    if mentions.group(1) in {"everyone", "here"} or mentions.group(1).startswith("&"):
                        ...
                        logger.debug(
                            "Checking all mentions, author: %s.", perms_author.mention_everyone)
            ...
            # If failed
            embed.set_footer(
                text='Click the "Edit" button below to edit your form.')
            await interaction.response.send_message(
                embed=embed,
                view=ScheduleEditView(
                    self.scheduler, interaction.user, self.channel, RawScheduleModalValues.from_modal(
                        self)
                ),
                ephemeral=True,
            )
    ...
    class ScheduleEditModal(discord.ui.Modal, title="Schedule Editor"):
        ...
        def sanitize_response(self, interaction: discord.Interaction) -> SanitizedScheduleEvent:
            ...
            if self.message.value is None:
                raise ValueError(
                    "Message cannot be None here since they are non-optional.")
    ...
    async def edit(self, interaction: discord.Interaction, button: discord.ui.Button[ScheduleEditView]) -> None:
        ...
        if self.original_event is None:
            ...
            await interaction.response.send_modal(
                get_schedule_modal(self.raw_values)(
                    self.scheduler, self.channel)
            )
        else:
            logger.info(
                "Creating a schedule edit modal from edit schedule view.")
            await interaction.response.send_modal(
                get_schedule_edit_modal(self.raw_values)(
                    self.scheduler, self.channel, self.original_event)
            )
        self.stop()
...
class ScheduleListView(discord.ui.View):
    ...
    async def render(self, interaction: discord.Interaction | commands.Context[Bot]) -> None:
        ...
        if interaction.guild is None:
            # TODO: support DM command - list all schedules
            raise ValueError("Guild shouldn't be None here.")

        logger.info(
            "Rendering list view for %s at %s, page idx: %d.", self.author, self.channel, self.current_page
        )

        author = interaction.author if isinstance(
            interaction, commands.Context) else interaction.user
        params = {
            "author_id": author.id,
        ...
        if total_count == 0:  # no scheduled messages
            ...
            if self.channel is None:
                embed = discord.Embed(
                    description="You have no scheduled messages.", colour=COLOUR)
            else:
                embed = discord.Embed(
        ...
        if start_index >= total_count:  # if the index is beyond the total count, goto last page
            self.current_page = ceil(
                total_count / self.LIMIT_PER_PAGE) - 1  # get last page
            start_index = self.current_page * self.LIMIT_PER_PAGE

        logger.debug("Querying schedule from %d with limit of %d.",
                     start_index, self.LIMIT_PER_PAGE)
        ...
        async with self.scheduler.db.execute(
            self.raw_query.format(
                limit=self.LIMIT_PER_PAGE, offset=start_index),
            params,
        ) as cur:
        ...
        if total_count > self.LIMIT_PER_PAGE:  # more than one page
            kwargs["view"] = self
            embed.set_footer(
                text=f"Page {self.current_page + 1} of {ceil(total_count / self.LIMIT_PER_PAGE)}")

        if self.responded:
            if isinstance(interaction, commands.Context):
                raise ValueError(
                    "Should only be interaction here (from button click).")
...
class Scheduler(Cog):
    ...
    async def _update_to_version_0(self) -> None:
        ...
        """
        logger.info("[orange]Updating DB version to 0[/orange]",
                    extra={"markup": True})
        async with self.db.execute(
            r"""
    ...
    async def _update_to_version_1(self) -> None:
        ...
        """
        logger.info("[orange]Updating DB version to 1[/orange]",
                    extra={"markup": True})
        async with self.db.execute(
            r"""
    ...
    if version.parse(aiosqlite.sqlite_version) >= version.parse("3.35.0"):
        ...
        async def _insert_schedule(self, event: ScheduleEvent) -> SavedScheduleEvent:
            ...
            async with self.db.execute(
                ...
                if row is None:
                    raise ValueError(
                        "Something went wrong with SQLite, row should not be None.")
    ...
    def _make_info_embed(event: SavedScheduleEvent) -> discord.Embed:
        ...
        if mentions is not None:  # has mentions
            embed.add_field(name="Ping Enabled",
                            value="Yes" if event.mention else "No", inline=True)
        timestamp = int(event.next_event_time)
        embed.add_field(
            name="Time", value=f"<t:{timestamp}> (<t:{timestamp}:R>)", inline=False)
        return embed
    ...
    async def save_event(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Something went wrong while saving event. Code: %s.", err_code, exc_info=e)
            embed = discord.Embed(
        ...
        if not editing:
            ...
            embed.set_footer(
                text=f"{event.author} has created a scheduled message.")
        else:
            embed.title = f"Scheduled Message Edited (Event ID #{event_db.id})"
            embed.set_footer(
                text=f"{event.author} has edited a scheduled message.")
        await interaction.response.send_message(embed=embed)
        return
    ...
    async def _save_event(
        ...
        if editing and original_event is None:
            raise ValueError(
                "original_event must not be None when editing is True.")
        ...
        if editing:
            ...
            logger.info("Edited schedule in database with ID %d.",
                        original_event.id)
            logger.info(
                "Message (preview): %s\nGuild: %s\nChannel: %s\nAuthor: %s\nRepeat: %s\nMention: %s\nTime: %s",
                event.message[:80],
    ...
    async def send_scheduled_message(self, stripped_event: StrippedSavedScheduleEvent) -> bool:
        ...
        if not hasattr(channel, "send"):
            logger.warning(
                "Event with ID %d channel is not a messageable channel.", event.id)
            return False
        ...
        if not perms_author.read_messages or not perms_author.send_messages:
            logger.warning(
                "Event with ID %d author doesn't have perms.", event.id)
            return False
        ...
        if not perms_bot.read_messages or not perms_bot.send_messages:
            logger.warning(
                "Event with ID %d bot doesn't have perms.", event.id)
            return False
            ...
            if self.schedule_heap:
                ...
                if next_event.next_event_time < now:
                    ...
                    if not success or next_event.repeat is None:
                        ...
                        if not success:
                            logger.info(
                                "Canceled %s because it failed.", next_event)
                        ...
                        async with self.db.execute(
                            r"""
                                UPDATE Scheduler
                                    ...
                                    WHERE id=$id
                            """,
                            {"next_event_time": new_event.next_event_time,
                                "id": next_event.id},
                        ):
                            pass
                        await self.db.commit()
    ...
    async def scheduler_event_loop(self) -> None:
        ...
        while not self.bot.is_closed():
            try:
                ...
                logger.error(
                    "An uncaught error was raised during scheduled event loop.", exc_info=e)
    ...
    async def _schedule_create(self, ctx: commands.Context[Bot], channel: MessageableGuildChannel | None) -> None:
        ...
        if not perms.read_messages or not perms.send_messages:
            logger.debug(
                "Author has no send or read messages perms for create.")
            embed = discord.Embed(
        ...
        if not perms.read_messages or not perms.send_messages:
            ...
            embed = discord.Embed(
                description=f"I don't have permission in {channel.mention}.", colour=COLOUR)
            await ctx.reply(embed=embed)
            return
    ...
    async def schedule(
        ...
        *,
        # type: ignore[reportGeneralTypeIssues]
        channel: MessageableGuildChannel = None,
    ) -> None:
    ...
    async def schedule_edit(
        ...
        event_id: int,
        # type: ignore[reportGeneralTypeIssues]
        new_channel: MessageableGuildChannel = None,
    ) -> None:
        ...
        if new_channel is not None:
            ...
            if not perms.read_messages or not perms.send_messages:
                logger.debug(
                    "Author has no send or read messages perms for edit channel.")
                embed = discord.Embed(
            ...
            if not perms.read_messages or not perms.send_messages:
                logger.debug(
                    "Bot has no send or read messages perms for edit channel.")
                embed = discord.Embed(
        ...
        if new_channel is None:
            # Check if channel still exists
            new_channel = cast(MessageableGuildChannel, ctx.guild.get_channel_or_thread(
                original_event.channel_id))
            if not new_channel:
                logger.warning(
                    "Event with ID %d channel not found.", original_event.id)
                embed = discord.Embed(
            ...
            if not hasattr(new_channel, "send"):
                logger.warning(
                    "Event with ID %d channel is not a messageable channel.", original_event.id)
                embed = discord.Embed(
                    description=f"It seems the original channel <#{original_event.channel_id}> no longer exists.",
                    colour=COLOUR,
                )
        ...
        if ctx.interaction is None:
            ...
            await ctx.reply(
                embed=embed,
                view=ScheduleEditView(
                    self, ctx.author, new_channel, raw_values, original_event=original_event),
            )
        else:
            # Otherwise, directly open the modal
            await ctx.interaction.response.send_modal(
                get_schedule_edit_modal(raw_values)(
                    self, new_channel, original_event)
            )
    ...
    async def schedule_delete(self, ctx: commands.Context[Bot], event_id: int) -> None:
        ...
        `event_id` - The event ID of the scheduled message (see `/list`).
        """
        logger.debug("%s is trying to delete schedule %d.",
                     ctx.author, event_id)
        ...
        # don't show for slash commands
        await ctx.reply(embed=embed, ephemeral=ctx.interaction is not None)
        return
