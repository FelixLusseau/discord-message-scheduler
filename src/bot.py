...
class Bot(commands.Bot):
    ...
    def __init__(self) -> None:
        ...
        activity = discord.Activity(
            name=f"avec vos programmations", type=discord.ActivityType.playing)
        ...
        logger.info("[bold green]Bot version: %s[/bold green]",
                    self.version, extra={"markup": True})
        ...
        if SYNC_SLASH_COMMANDS:
            ...
            if not DEBUG_MODE:
                ...
                for guild_id in DEBUG_GUILDS:
                    ...
                    await self.tree.sync(guild=discord.Object(guild_id))
                    logger.info(
                        "Synced app command tree to debug guild %d.", guild_id)
        else:
    ...
    async def on_ready(self) -> None:
        ...
        """
        logger.info("[bold green]Bot is ready.[/bold green]",
                    extra={"markup": True})
    ...
    async def on_command_error(  # type: ignore[reportIncompatibleMethodOverride]
        ...
        if isinstance(exception, commands.BadUnionArgument):
            ...
            if len(to_string) > 2:
                fmt = "{}, or {}".format(
                    ", ".join(to_string[:-1]), to_string[-1])
            else:
                fmt = " or ".join(to_string)
            arg = discord.utils.escape_markdown(
                context.current_argument or "", ignore_links=False)
            embed = discord.Embed(
                description=f"**{arg}** is not a valid {fmt}.", colour=COLOUR)
